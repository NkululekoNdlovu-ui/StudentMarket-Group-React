...
export const capturedProductDetails = (formData) => {
  ...
  console.log(
    "ProductService capturedProductDetails - Token:",
    token ? `${token.substring(0, 20)}...` : "No valid token found"
  );
...
export const payForProduct = (product) => {
  ...
  console.log(
    "ProductService payForProduct - ProductID:",
    productId,
    "Token:",
  ...
  return axios.post(
    `${PRODUCT_API_BASE_URL}/checkout/${productId}`,
    {},
    {
      headers: {
        Authorization: `Bearer ${token}`,
